...
if (isset($_SESSION['uid'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $query = "SELECT CONCAT(`fname`, ' ', `lname`) AS full_name FROM `admins` WHERE `admins`.`id`=? UNION SELECT CONCAT(`full_name_arabic`, ' ', `address`) AS full_name FROM `students`  WHERE `students`.`id`=? ; ";
